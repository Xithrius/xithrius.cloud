...
<!-- <template>
  ...
  >
    <div
      class="flex items-cente justify-between py-2 px-6 md:flex-1 dark:bg-gray-900 bg-zinc-200 rounded-lg"
    >
      <nav class="hidden md:flex md:space-x-4 lg:space-x-6" aria-label="Global">
        <NavigationMenuItem v-for="item in items" :key="item.name" :item="item" />
      </nav>
